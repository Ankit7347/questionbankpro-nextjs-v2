...
import { ExamUI, EducationLevelGroupUI } from "@/dto/exam.ui.dto";
 ...
 */
export async function fetchExamCatalog(): Promise<ApiResponseUI<EducationLevelGroupUI[]>> {
  ...
  try {
    const res = await fetch(`${baseUrl}/exams/catalog`, {
      // 2. Add caching config here
      next: { 
        revalidate:revalidateTime , // Re-fetch data at most every hour
        tags: ["exams-catalog"] // Tag for manual cache clearing
      }
    });
 ...
 * Flat list
 */
export async function fetchExamLanding(): Promise<ApiResponseUI<ExamUI[]>> {
  // 1. Get the base URL (Use environment variable for production)
  const baseUrl = process.env.NEXT_PUBLIC_API_URL || "http://localhost:3000";
  const revalidateTime = Number(process.env.NEXT_PUBLIC_API_REVALIDATE) || 3600;
  try {
