...
async function ChapterNotesPage({ params }: PageProps) {
  ...
  if (error || !data) {
    return (
      <div className="bg-white dark:bg-slate-950 text-slate-800 dark:text-slate-200 p-4 md:p-6 pb-24">
        <div className="text-center">
          <p className="text-red-500 dark:text-red-400">Failed to load chapter topics</p>
      ...
      <header className="mb-8">
        ...
        <div className="flex justify-between items-end">
          ...
          <div className="text-right hidden md:block">
            ...
            <div className="text-xs text-slate-400 dark:text-slate-500">Topics Completed</div>
          </div>
        </div>
        ...
        <div className="mt-4 grid grid-cols-2 md:grid-cols-3 gap-4">
          <div className="bg-slate-50 dark:bg-slate-900/50 border border-slate-200 dark:border-slate-800 rounded-lg p-3">
            ...
            <p className="text-xl font-bold text-cyan-600 dark:text-cyan-400">{stats.totalTopics}</p>
          </div>
          <div className="bg-slate-50 dark:bg-slate-900/50 border border-slate-200 dark:border-slate-800 rounded-lg p-3">
      ...
      <div className="space-y-4">
        {topics.length === 0 ? (
          <div className="text-center py-16">
            ...
            <p className="text-sm text-slate-400 dark:text-slate-500 mt-2">Check back soon for more content</p>
          </div>
        ) : (
          topics.map((topic: TopicData) => (
            ...
            >
              <div className="group m-2 bg-slate-50 dark:bg-slate-900/40 border border-slate-200 dark:border-slate-800 rounded-xl p-5 hover:border-cyan-500/50 dark:hover:border-cyan-500/30 hover:bg-slate-100 dark:hover:bg-slate-900/60 transition-all cursor-pointer">
                <div className="flex items-start gap-4">
                  ...
                  <div className="flex-1">
                    <div className="flex justify-between items-start">
                      ...
                      <span className="text-xs text-slate-500 dark:text-slate-500 bg-slate-100 dark:bg-slate-950 px-2 py-1 rounded border border-slate-200 dark:border-slate-800">
