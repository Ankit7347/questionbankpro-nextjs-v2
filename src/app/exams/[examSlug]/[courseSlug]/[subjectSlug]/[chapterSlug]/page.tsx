...
import { 
  ...
  MessageSquare,
  Clock,
  ExternalLink // Added icon for clarity
} from "lucide-react";
...
export default function ChapterPage({ 
  ...
  const completedCount = data.topics.filter(t => t.isCompleted).length;
    ...
    <div className="flex flex-col lg:flex-row min-h-[calc(100vh-64px)] bg-white dark:bg-slate-950">
      ...
      <main className="flex-1 overflow-y-auto border-r border-gray-100 dark:border-slate-900">
        <div className="p-4 border-b border-gray-50 dark:border-slate-900 flex items-center justify-between bg-gray-50/50 dark:bg-slate-900/50">
          ...
          <div className="text-xs font-bold text-gray-400 uppercase tracking-widest">
            Topic {activeTopicIndex + 1} of {totalTopics}
          </div>
        </div>
        ...
        <div className="p-6 lg:p-10 max-w-4xl mx-auto">
          {activeTopic.type === "video" ? (
            ...
            <div className="aspect-video w-full bg-blue-50 dark:bg-blue-900/10 rounded-3xl border-2 border-dashed border-blue-200 dark:border-blue-800 flex flex-col items-center justify-center p-10 text-center">
              ...
              <p className="text-gray-500 max-w-xs mt-2">This topic contains PDF notes for {activeTopic.title}.</p>
          ...
          <div className="mt-10 prose dark:prose-invert max-w-none">
            ...
            <p className="text-gray-600 dark:text-gray-400">
              In this session of {data.title}, we focus specifically on {activeTopic.title}. 
              This is a critical part of the {subject.name} syllabus for {exam.name}.
            </p>
          </div>
        </div>
      ...
      <aside className="w-full lg:w-80 xl:w-96 bg-gray-50/50 dark:bg-slate-900/30 overflow-y-auto">
        <div className="p-6 border-b border-gray-100 dark:border-slate-800">
          ...
          <p className="text-[10px] font-bold text-gray-400 mt-2 uppercase tracking-widest">{progressPercent}% Completed</p>
        </div>
        ...
        <nav>
          {data.topics.map((topic, index) => (
            <div
              key={topic.id}
              className={`group w-full flex items-start gap-4 p-5 text-left border-b border-gray-100 dark:border-slate-800/50 transition-all ${
                activeTopicIndex === index 
                  ? "bg-white dark:bg-slate-800 shadow-sm z-10" 
                  : "hover:bg-gray-100/50 dark:hover:bg-slate-800/30"
              }`}
            >
              <div className="mt-1" onClick={() => setActiveTopicIndex(index)}>
                {topic.isCompleted ? (
              ...
              <div className="flex-1 min-w-0">
                {/* Clicking Title updates the Video Player (Left Side) */}
                <button
                  onClick={() => setActiveTopicIndex(index)}
                  className={`block w-full text-left text-sm font-bold truncate ${activeTopicIndex === index ? 'text-blue-600' : 'text-gray-700 dark:text-gray-300 hover:text-blue-500'}`}
                >
                  {topic.title}
                </button>
